                ...
                <table style="width: 100%;">
                    ...
                    @foreach ($comments as $comment)
                        <tr class="border">
                            ...
                            <td class="border">
                                <div class="flex">
                                    <form action="{{url('admin/report/reply-comment/detail')}}" method="get">
                                        @csrf
                                        <input type="hidden" name="id" value="{{$comment->reported_reply_comment_id}}">
                                        <button type="submit" class="py-2 px-4 bg-blue-400 rounded text-white mr-1"><i class="fas fa-info-circle"></i></button>
                                    </form>
                                    <form action="{{url('admin/report/reply-comment/ignore')}}" method="post">
                                        @csrf 
                                        @method('delete')
                                        <input type="hidden" name="id" value="{{$comment->reported_reply_comment_id}}">
                                        <button type="submit" class="py-2 px-4 bg-green-400 rounded text-white"><i class="fas fa-check-circle"></i></button>
                                    </form>
                                    <form action="{{url('admin/report/reply-comment/delete')}}" method="post">
                                        @csrf 
                                        @method('delete')
                                        <input type="hidden" name="id" value="{{$comment->reported_reply_comment_id}}">
